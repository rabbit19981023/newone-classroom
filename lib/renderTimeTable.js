...
class AddStrategy {
  use (req, res, data) {
    ...
    RoomsListModel.findAll(async (error, rooms) => {
      ...
      RoomsTimeModel.findAll({ roomName: roomName }, async (error, roomTime) => {
...
class DeleteStrategy {
  use (req, res, data) {
    ...
    RoomsTimeModel.findAll(async (error, roomsTime) => {
      ...
      RoomsTimeModel.findMany({ roomName: roomName }, async (error, roomTime) => {
        if (error) { res.render('500error', { layout: 'error' }) }
...
class ReserveStrategy {
  use (req, res, data) {
    ...
    RoomsTimeModel.findAll(async (error, roomsTime) => {
      ...
      RoomsTimeModel.findMany({ roomName: roomName, roomWeek: roomWeek }, async (error, roomTime) => {
        if (error) { return res.render('500error') }
